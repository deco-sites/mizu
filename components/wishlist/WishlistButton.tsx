...
function WishlistButton({ item, variant = "full" }: Props) {
    ...
    return (<>
      <button id={id} data-wishlist-button disabled {...addToWishlistEvent} aria-label="Add to wishlist" hx-on:click={useScript(onClick, productID, productGroupID)} class={clx("btn no-animation  ml-auto hover:bg-transparent", variant === "icon"
            ? "btn-circle btn-ghost btn-sm"
            : "btn-primary btn-outline gap-2 w-full")}>
        <Image src={"https://mizunobrio.vtexassets.com/assets/vtex/assets-builder/mizunobrio.store-theme/4.0.39/icons/icon-wishlist___aac3488ee8d3cf3a48c551f93bebb442.png"} width={20} height={21} class={""}/>
