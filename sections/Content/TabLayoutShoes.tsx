...
export default function TabLayoutShoes({
  ...
  mobileImage = 'https://ozksgdmyrqcxcwhnbepg.supabase.co/storage/v1/object/public/assets/11899/f1b4ea9b-438f-445a-9786-377d32280279',
  tabs,
  selected = 1
  ...
  return (
    <div class="bg-[#f2f2f2] lg:bg-white flex flex-col lg:items-center px-[12px] lg:px-0 mt-[40px] mb-[50px] mb-[24px] py-[12px] lg:py-[15px]">
      <h1 class="max-w-[190px] mt-[26px] mx-[14px] mb-[40px] lg:m-0 lg:max-w-[1240px] w-full font-black text-primary leading-[37.5px] text-[32px] lg:text-[40px] lg:leading-[46.88px]">{title}</h1>
              ...
              tabs.map((tab, index) => (
                ...
                >
                  <Image
                    ...
                    loading="lazy"
                  />
                  <p class="text-left text-[#060606] leading-[17px]" style={{ display: index == selected ? 'flex' : 'none' }}>{tab.description}</p>
                </button>
              ))
            }
          </div>
            ...
            tabs.map(tab => (
              <div class="flex flex-col items-center justify-start">
                <Image
                  ...
                  loading="lazy"
                />  
                <p class="text-[12px] font-roboto leading-[14px] text-[#060606]">{tab.description}</p>
              </div>
            ))
          }
        </div>
